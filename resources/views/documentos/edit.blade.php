                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="categoria_id" id="categoria_id" size="5"
                            ...
                            @foreach($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ old('categoria_id', $documento->categoria_id) == $categoria->id ? 'selected' : '' }} class="py-1">
                                    {{ $categoria->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="etiquetas[]" id="etiquetas" multiple size="5"
                            class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-sky-400 focus:border-transparent transition overflow-y-auto">
                            @foreach($etiquetas as $etiqueta)
                                <option value="{{ $etiqueta->id }}" {{ $documento->etiquetas->contains($etiqueta->id) ? 'selected' : '' }} class="py-1">
                                    #{{ $etiqueta->nombre }}
                                </option>
                            @endforeach
                        </select>
